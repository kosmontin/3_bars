...
def get_closest_bar(bars, longitude=37.620374, latitude=55.753556):
    return sorted(bars, key=lambda bar: math.sqrt(
        (abs(bar['geometry']['coordinates'][0] - longitude) ** 2) +
...
if __name__ == '__main__':
    ...
    значит скрипт запущен с параметрами по умолчанию.
    Файл bars.json, начальные координаты: Красная Площадь.
    ''')
    print('Самый маленький бар:',
          get_smallest_bar(load_data())
          ['properties']['Attributes']['Name'],
          'Количество мест: ',
          get_smallest_bar(load_data())
          ['properties']['Attributes']['SeatsCount'])
    print('Самый большой бар:',
          get_biggest_bar(load_data())
          ['properties']['Attributes']['Name'],
          'Количество мест: ',
          get_biggest_bar(load_data())
          ['properties']['Attributes']['SeatsCount'])
    print('Ближайший бар:',
          get_closest_bar(load_data())[0]
          ['properties']['Attributes']['Name'],
          '\nНаходится по адресу: ',
          get_closest_bar(load_data())[0]
